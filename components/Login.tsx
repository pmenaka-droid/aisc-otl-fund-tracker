...
// Refined Props interface to match App.tsx requirements for Gmail API access
interface Props {
...
const Login: React.FC<Props> = ({ onLogin }) => {
  ...
  // Handles the OAuth2 flow using Google Identity Services to get access tokens
  // Fixes the type mismatch in App.tsx by providing a structured userData object
  const handleAuth = () => {
    if (!clientId) {
    ...
    try {
      ...
      const client = window.google.accounts.oauth2.initTokenClient({
        ...
        callback: async (response: any) => {
          if (response.error) {
            setError(`Authorization failed: ${response.error_description || response.error}. Ensure Gmail API is enabled in Cloud Console.`);
            setIsLoading(false);
            return;
          }
          ...
          try {
            ...
            setError("Failed to retrieve user profile information from Google.");
            setIsLoading(false);
          }
        },
      });
      ...
      setError("OAuth initialization failed. Check your Client ID configuration in settings.");
      setIsLoading(false);
    }
  };
  ...
  const saveConfig = (e: React.FormEvent) => {
    ...
    if (!clientId.trim() || !clientId.includes('.apps.googleusercontent.com')) {
      alert("Please enter a valid Google Client ID (usually ends in .apps.googleusercontent.com)");
      return;
    }
    localStorage.setItem('google_sso_client_id', clientId.trim());
